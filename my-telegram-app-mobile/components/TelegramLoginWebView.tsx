    ...
    const handleNavigationStateChange = (navState: WebViewNavigation) => {
        ...
        if (url.includes('auth-callback') || url.includes('mytelegramappmobile://auth')) {
            try {
                ...
                console.error('[TelegramLoginWebView] Failed to parse auth callback:', error);
            }
        }
    };
    ...
    const handleMessage = (event: any) => {
        try {
            ...
            if (data.type === 'telegram_auth' && data.user) {
                console.log('[TelegramLoginWebView] Received auth data from WebView:', data.user);
                onLoginSuccess(data.user);
            }
        } catch (error) {
            console.error('[TelegramLoginWebView] Failed to parse message:', error);
        }
    };
