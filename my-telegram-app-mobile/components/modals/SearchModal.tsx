...
import { Search, X, Package, Tag, Factory, ArrowRight, ShoppingBag } from 'lucide-react-native';
...
export default function SearchModal({ visible, onClose }: SearchModalProps) {
    ...
    const renderProductItem = useCallback(({ item }: { item: any }) => (
        ...
        />
    ), [toggleFavorite, handleShowProductDetails, favoriteIds]);
    ...
    const renderContent = () => {
        ...
        const hasResults = (searchResults.products?.totalItems || 0) > 0 || (searchResults.deals?.length || 0) > 0 || (searchResults.suppliers?.length || 0) > 0;
                        ...
                        {TABS.map((tab) => {
                            ...
                            return (
                                <TouchableOpacity
                                    ...
                                    className={`flex-1 flex-row items-center justify-center py-2.5 mx-1 rounded-xl border ${isActive
