...
import { Search, Settings, LogIn } from "lucide-react-native"
      ...
      <View className="px-4 py-3 flex-row justify-between items-center">
        ...
        <View className="flex-row items-center gap-3">
          ...
          <PressableScale
            ...
            haptic="selection"
            hitSlop={{ top: 6, bottom: 6, left: 6, right: 6 }}
            className={`w-10 h-10 rounded-xl items-center justify-center border ${isSettingsRoute ? "bg-gray-100 border-gray-200" : "bg-surface border-border"
              }`}
          >
          ...
          {isGuest ? (
            <PressableScale
              ...
              haptic="selection"
              hitSlop={{ top: 6, bottom: 6, left: 6, right: 6 }}
              className="h-10 bg-primary-50 rounded-xl items-center justify-center border border-primary-200 px-3 flex-row gap-1.5"
            >
              <LogIn size={16} color="#2563EB" />
              <Text className="text-primary-600 font-bold text-sm">دخول</Text>
            </PressableScale>
          ) : (
            <PressableScale
