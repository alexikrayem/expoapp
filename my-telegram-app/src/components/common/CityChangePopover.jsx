...
// ✅ Simple in-memory cache to keep cities for later openings
let cachedCities = null;

const CityChangePopover = ({ onCitySelect, currentCityId, onClose, preloadedCities }) => {
  const [availableCities, setAvailableCities] = useState(preloadedCities || cachedCities || []);
  const [isLoading, setIsLoading] = useState(!preloadedCities && !cachedCities);
  ...
  useEffect(() => {
    // ✅ If we already have cached cities, skip loading
    if (cachedCities) return;

    const fetchCities = async () => {
      try {
        const data = await cityService.getCities();
        cachedCities = data; // ✅ Store in memory for instant reuse
      ...
      >
        <div className="p-2">
          ...
          <div className="max-h-60 overflow-y-auto">
            {isLoading ? (
              // ✅ Show skeleton immediately but still render the popover
