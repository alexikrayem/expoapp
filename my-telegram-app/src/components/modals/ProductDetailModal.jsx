...
import { X, ShoppingCart, Heart, AlertTriangle, ChevronsRight, Loader2, Maximize2 } from "lucide-react"
  ...
  return (
    <>
        ...
        <div className="flex-grow p-4 md:p-6">
          ...
          {!isLoading && !error && originalProduct && (
            <div className="space-y-6">
              {!isAvailable && (
                <div
                  className="bg-yellow-100 border-l-4 border-yellow-500 text-yellow-800 p-4 rounded-r-lg"
                  role="alert"
                >
              ...
              <div className="relative">
                <div
                  className={`w-full h-64 sm:h-80 bg-gray-200 rounded-lg flex items-center justify-center overflow-hidden shadow bg-cover bg-center ${!isAvailable ? "grayscale" : ""}`}
                  style={{ backgroundImage: `url(${originalProduct.image_url})` }}
                ></div>
                <button
                  onClick={() => {
              ...
              <div className="space-y-3">
                ...
                <div className="flex flex-wrap items-baseline gap-x-4 gap-y-1">
                  <p
                    className={`text-4xl font-extrabold ${isAvailable ? "text-blue-600" : "text-gray-400 line-through"}`}
                  >
                    {formatPrice(originalProduct.effective_selling_price)}
                  </p>
                </div>
              </div>
              ...
              <div className="pt-6 border-t flex items-center gap-3">
                <button
                  ...
                  className="flex-grow bg-blue-500 text-white py-3.5 px-6 rounded-lg font-semibold hover:bg-blue-600 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2 text-lg"
                >
                  <ShoppingCart className="h-5 w-5" /> {isAvailable ? "إضافة للسلة" : "غير متوفر"}
                </button>
                <button
                  onClick={handleToggleFavorite}
                  className="p-3.5 border border-gray-300 rounded-lg text-gray-600 hover:border-red-500 hover:text-red-500"
                >
      ...
      <ImageViewer
        ...
        imageName={originalProduct?.name}
