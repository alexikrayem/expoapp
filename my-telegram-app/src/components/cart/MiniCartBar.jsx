// src/components/cart/MiniCartBar.jsx (SIMPLIFIED VERSION)
...
const MiniCartBar = () => {
    ...
    if (showActiveItemControls && activeMiniCartItem) {
        ...
        displayItem = currentItemInCart || { 
            ...activeMiniCartItem, 
            quantity: 1, 
            product_id: activeItemId 
        };
    }
            ...
            <div className="w-full max-w-4xl px-3 sm:px-4 pointer-events-auto">
                <AnimatePresence>
                    {displayItem && (
                        ...
                        >
                            <button 
                                onClick={hideActiveItemControls} 
                                className="absolute top-2 left-2 p-1 text-gray-400"
                            >
                                <X size={16}/>
                            </button>
                            <div className="flex items-center justify-between gap-3 pl-8">
                                ...
                                <div className="flex items-center gap-2 flex-shrink-0">
                                    <button 
                                        onClick={() => actions.decreaseQuantity(displayItem.product_id)} 
                                        className="p-2 bg-gray-100 rounded-lg"
                                    >
                                        <Minus size={16}/>
                                    </button>
                                    <span className="text-md font-semibold w-7 text-center">
                                        {displayItem.quantity}
                                    </span>
                                    <button 
                                        onClick={() => actions.increaseQuantity(displayItem.product_id)} 
                                        className="p-2 bg-gray-100 rounded-lg"
                                    >
                                        <Plus size={16}/>
                                    </button>
                ...
                <motion.div
                    ...
                    className={`bg-blue-600 text-white shadow-xl p-3.5 w-full transition-all duration-300 ${
                        displayItem ? 'rounded-b-xl' : 'rounded-xl'
                    }`}
                >
